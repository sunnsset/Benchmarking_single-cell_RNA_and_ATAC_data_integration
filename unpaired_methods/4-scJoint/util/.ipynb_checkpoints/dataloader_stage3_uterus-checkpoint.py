...
#     print('load mtx matrix:', file_name)
#     data = sc.read(file_name, cache = True).T
#     data = data.X
#     # data = data.todense()
...
def npz_reader_new(file_name):
    print('load mtx matrix:', file_name)
    data = sc.read(file_name, cache = True).X
    ...
    def __init__(self, config):
        ...
        if len(config.atac_paths) == len(config.atac_protein_paths):
            for i, (atac_path, atac_protein_path) in enumerate(zip(config.atac_paths, config.atac_protein_paths)): 
                pseudo_label_path = '/home/xcx/results/uterus/4-scJoint/' + os.path.basename(config.atac_paths[i]).split('.')[0] + '_knn_predictions.txt'

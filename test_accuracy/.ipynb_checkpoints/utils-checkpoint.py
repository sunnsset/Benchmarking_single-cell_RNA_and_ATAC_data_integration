...
def plot_latent(z1, z2, anno1 = None, anno2 = None, mode = "joint", save = None, figsize = (20,10), axis_label = "Latent", colors=None, **kwargs):
    """\
    Description
        Plot latent space
    Parameters
    ...
    if mode == "modality":
        ...
        axs.legend(loc='upper left', frameon = False, ncol = 1, bbox_to_anchor=(0.9, 1), markerscale = _kwargs["markerscale"], fontsize = 40)
        axs.set_title("omics layer", fontsize = 40)
        
        axs.tick_params(axis = "both", which = "major", width=3, labelsize = 25)
    ...
    elif mode == "modality_1":
        colormap = plt.cm.get_cmap("tab10")
        axs = fig.add_subplot()
        axs.scatter(z1[:,0], z1[:,1], color = colormap(1), label = "scATAC-Seq", s = _kwargs["s"], alpha = _kwargs["alpha"])
        axs.scatter(z2[:,0], z2[:,1], color = colormap(2), label = "scRNA-Seq", s = _kwargs["s"], alpha = _kwargs["alpha"])
        axs.legend(loc='upper left', frameon = False, ncol = 1, bbox_to_anchor=(0.9, 1), markerscale = _kwargs["markerscale"], fontsize = 40)
        axs.set_title("omics layer", fontsize = 40)
        ...
        axs.set_title("trajectory", fontsize = 40)
        ...
        for i in range(len(center_list_x)):
            axs.scatter(center_list_x[i], center_list_y[i], color = "k", linewidths=7)
        ...
        # 计算点之间的距离
        distances = squareform(pdist(center_list))
        ...
        for i in edges:
            # print(i[0], i[1])
            plt.plot([center_list_x[i[0]], center_list_x[i[1]]], [center_list_y[i[0]], center_list_y[i[1]]], 'k--', linewidth= 4.0)
        
        axs.legend(loc='upper left', frameon = False, ncol = 1, bbox_to_anchor=(1, 1), markerscale = _kwargs["markerscale"], fontsize = 25)
...
#         ax.spines['top'].set_visible(False) 
        ...
        for i, cluster_type in enumerate(cluster_types):
            ...
            if index.shape[0] != 0:
                axs.scatter(z1[index,0], z1[index,1], color = colormap[i], label = cluster_type, s = _kwargs["s"], alpha = _kwargs["alpha"])
                axs.scatter(z2[index,0], z2[index,1], color = colormap[i], s = 30, alpha = _kwargs["alpha"])
        axs.legend(loc='upper left', frameon = False, ncol = 1, bbox_to_anchor=(1, 1), markerscale = _kwargs["markerscale"], fontsize = 20)
        axs.set_title("cell type", fontsize = 40)

        axs.tick_params(axis = "both", which = "major", width=3, labelsize = 25)

        axs.set_xlabel(axis_label + " 1", fontsize = 19)
        axs.set_ylabel(axis_label + " 2", fontsize = 19)
        # axs.set_xlim(z1[:,0], z1[:,0])
        # axs.set_ylim(z1[:,1], z1[:,1])
        axs.spines['right'].set_visible(False)
        axs.spines['top'].set_visible(False)  
        axs.spines['left'].set_linewidth(2)
        axs.spines['bottom'].set_linewidth(2)
        ...
        axs[0].legend(loc='upper left', frameon = False, ncol = 1, bbox_to_anchor=(1, 1), markerscale = _kwargs["markerscale"], fontsize = 20)
        axs[0].set_title("scRNA-Seq cell type", fontsize = 40)

        axs[0].tick_params(axis = "both", which = "major", width=3, labelsize = 25)
        ...
        # axs[1].axis("off")
        axs[1].legend(loc='upper left', frameon = False, ncol = 1, bbox_to_anchor=(1.04, 1), markerscale = _kwargs["markerscale"], fontsize = 20)
        axs[1].set_title("scATAC-Seq cell type", fontsize = 40)

        axs[1].tick_params(axis = "both", which = "major", width=3, labelsize = 15)

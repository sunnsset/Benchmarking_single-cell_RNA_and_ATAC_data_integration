...
from util.dataloader_stage1_uterus import PrepareDataloader
...
class TrainingProcessStage1():
    ...
    def train(self, epoch):
        ...
        for batch_idx in range(self.training_iters):
            ...
            progress_bar(batch_idx, self.training_iters,'encoding_loss: %.3f, rna_loss: %.3f ' % (total_encoding_loss / (batch_idx + 1), total_cell_loss / (batch_idx + 1),))
        # save checkpoint
        save_checkpoint({
            'epoch': epoch,
    ...
    def write_embeddings(self):
        ...
        if not os.path.exists("/home/xcx/results/uterus/4-scJoint/"):
            os.makedirs("/home/xcx/results/uterus/4-scJoint/")
        
        # rna db
        for i, rna_loader in enumerate(self.test_rna_loaders):
            db_name = os.path.basename(self.config.rna_paths[i]).split('.')[0]
            fp_em = open('/home/xcx/results/uterus/4-scJoint/' + db_name + '_embeddings.txt', 'w')
            fp_pre = open('/home/xcx/results/uterus/4-scJoint/' + db_name + '_predictions.txt', 'w')

...
import pandas as pd
k1_matrix = pd.read_csv(f1, header=0, index_col=False).to_numpy()
k2_matrix = pd.read_csv(f2, header=0, index_col=False).to_numpy()

I_p=tf.eye(p)
record = open('loss_1469.txt', 'w')
...
for i in range(10001):
  sess.run(train_step)
  if (i%100 == 0): 
    np.savetxt("./train_1469/alpha_hat_"+str(k)+"_"+str(i)+".txt", sess.run(alpha))
    np.savetxt("./train_1469/beta_hat_"+str(k)+"_"+str(i)+".txt", sess.run(beta))
